function [packetSamples,packetCnt,syncTimeVec, pktSig] = ...
...
for p=0:numSubFrames-2
  ...
  if (syncTime > 0) && (syncTime+(adsbParam.MaxPacketLength-1) <= xLen)
    ...
    rxSyncSeq = sum(reshape(rxSyncSignal, spc, syncLen),1);
    
    % Check if this is a valid sync sequence. Get a threshold value based on
    % assumed high and low locations.
    highValue = sum(rxSyncSeq(adsbParam.SyncSequenceHighIndices)) / adsbParam.SyncSequenceNumHighValues;
    lowValue = sum(rxSyncSeq(adsbParam.SyncSequenceLowIndices)) / adsbParam.SyncSequenceNumLowValues;
    ...
    if all(xor((rxSyncSeq < th), adsbParam.SyncSequence))
      ...
      if packetCnt <= adsbParam.MaxNumPacketsInFrame
        ...
        % matrix
        dataIndices = int32(adsbParam.PreambleLength + (0:adsbParam.LongPacketLength-1));
        packetSamples(1:adsbParam.LongPacketLength,packetCnt) = xBuff(syncTime+dataIndices,1);
        pktSig = zBuff(syncTime+dataIndices,1);
